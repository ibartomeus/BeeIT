...
ITtongue <- function(IT, family, mouthpart = "all"){
  ...
  family_intercepts <- data.frame(families = c("Andrenidae", "Apidae", 
                                  ...
                                  intercepts_prementum = c(0.83, 1.03, 0.56, 0.90, 1.07),
                                  slopes_prementum = c(0.73, 1.14, 1.04, 0.68,0.73))
  ...
  tongue_f <- exp(log(family_intercepts2$intercepts_tongue)
                + 0.96*log(IT))  
  glossa_f <- exp(log(family_intercepts2$intercepts_glossa)
                + 1.04*log(IT))  
  prementum_f <- exp(log(family_intercepts2$intercepts_prementum)
                + family_intercepts2$slopes_prementum*log(IT))
  if (mouthpart == "all") out <- cbind(tongue_f, glossa_f, prementum_f)
  if (mouthpart == "tongue") out <- tongue_f
  if (mouthpart == "glossa") out <- glossa_f
